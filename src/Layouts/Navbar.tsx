...
function Navbar() {
  ...
  const setDefault = userStore((state) => state.setDefault);
      ...
      <div className="max-w-screen-xl flex flex-wrap items-center justify-between mx-auto p-4">
        ...
        >
          ...
          <ul className="flex flex-col p-4 md:p-0 mt-4 font-medium border border-gray-100 rounded-lg bg-gray-50 md:space-x-8 rtl:space-x-reverse md:flex-row md:mt-0 md:border-0 md:bg-white dark:bg-gray-800 md:dark:bg-gray-900 dark:border-gray-700">
            <li>
              <Dinamic_anchor href="/menu">Menu</Dinamic_anchor>
            </li>
            <li>
              <Dinamic_anchor href="/cart">Cart</Dinamic_anchor>
            </li>
            <li>
              {user.isAuthenticated == "AUTH" ? (
                <Dinamic_anchor href="/profile">
                  {user.username ?? "Profile"}
                </Dinamic_anchor>
              ) : (
                <Dinamic_anchor href="/auth">Sign in</Dinamic_anchor>
              )}
            </li>
            {user.isAuthenticated === "AUTH" && (
              <button
...
function Dinamic_anchor({
  ...
  if (isActive) {
    return (
      <a href={href} className={active} aria-current="page">
        {children}
      </a>
    );
  }
  return (
    <a href={href} className={unactive}>
      {children}
    </a>
  );
}
